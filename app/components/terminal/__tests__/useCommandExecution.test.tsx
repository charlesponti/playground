...
describe("useCommandExecution Hook", () => {
	...
	test("executeCommand should handle help command", () => {
		...
		expect(
			newLines.some((line: TerminalLine) =>
				line.content.includes("AVAILABLE COMMANDS"),
			),
		).toBe(true);
	});
	...
	test("executeCommand should handle meow command", () => {
		...
		expect(
			newLines.some((line: TerminalLine) =>
				line.content.includes("Summoning cyber cats"),
			),
		).toBe(true);

		expect(
			newLines.some((line: TerminalLine) =>
				line.content.includes("MEOW! Cats are bouncing"),
			),
		).toBe(true);

		// Should contain cat ASCII art
		expect(
			newLines.some((line: TerminalLine) => line.content.includes("/\\_/\\")),
		).toBe(true);
	});
	...
	test("executeCommand should handle unknown command", () => {
		...
		expect(
			newLines.some((line: TerminalLine) =>
				line.content.includes("Bad command or file name"),
			),
		).toBe(true);
	});
